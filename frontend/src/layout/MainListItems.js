...
import PluginMenuItem from "../components/PluginMenuItem";
...
const MainListItems = ({ collapsed, drawerClose }) => {
  ...
  const { getMenuItems: getPluginMenuItems } = usePlugins();
  ...
  return (
    <div onClick={drawerClose}>
      ...
      */}
      ...
      {/* 🧩 Seção de Plugins */}
      {getPluginMenuItems().length > 0 && (
        <>
          <Divider />
          <ListSubheader inset>
            {i18n.t("mainDrawer.listItems.plugins") || "Plugins"}
          </ListSubheader>
          {getPluginMenuItems().map((pluginMenu) => (
            <PluginMenuItem
              key={pluginMenu.route}
              plugin={pluginMenu}
              collapsed={collapsed}
            />
          ))}
