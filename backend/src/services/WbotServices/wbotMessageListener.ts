...
import TicketTimeoutService from "../TicketServices/TicketTimeoutService";
...
const verifyQueue = async (
  ...
  let randomUserId;
  let isImmediateRandomization = false;

  if (choosenQueue) {
    try {
      const userResult = await ListUserQueueImmediateService(choosenQueue.id, ticket.id);

      randomUserId = userResult.userId;
      isImmediateRandomization = userResult.isImmediate;

      // Se for randomização imediata e há usuário disponível, agendar timeout
      if (isImmediateRandomization && randomUserId) {
        const queue = await Queue.findByPk(choosenQueue.id);
        if (queue && queue.tempoRoteador > 0) {
          await TicketTimeoutService({
            ticketId: ticket.id,
            queueId: choosenQueue.id,
            companyId: ticket.companyId,
            timeoutMinutes: queue.tempoRoteador
          });
        }
      }
    } catch (error) {
      console.error("Erro ao selecionar usuário para fila:", error);
...
export const handleMessageIntegration = async (
  ...
  if (queueIntegration.type === "n8n" || queueIntegration.type === "webhook") {
    // --- INÍCIO LÓGICA WEBHOOKS_N8N MIGRADA DO BASE.JS ---
    const shouldSendAllMessages = process.env.WEBHOOKS_N8N === "true";
    const isFromMe = msg.key.fromMe;

    // Se WEBHOOKS_N8N=false, envia apenas mensagens do cliente
    if (!shouldSendAllMessages && isFromMe) {
      return;
    }

    // Verifica se o ticket tem integração ativa (apenas se WEBHOOKS_N8N=false)
    if (!shouldSendAllMessages && (!ticket.integrationId || !ticket.useIntegration)) {
      return;
    }
    ...
    try {
      ...
      if (msgType === "conversation" || msgType === "extendedTextMessage") {
        formattedBody = getBodyMessage(msg);
        if (formattedBody) {
          formattedBody = formattedBody.trim().replace(/\n\s*\n/g, '\n');
          formattedBody = formattedBody.replace(/\r\n/g, '\n');
        }
      }

      if (["audioMessage", "imageMessage", "videoMessage", "documentMessage", "stickerMessage"].includes(msgType)) {
        try {
          const media = await downloadMedia(msg, null, wbot);
          if (media && media.data) {
      ...
      let extraWhatsappData = {};
      // Sempre buscar e enviar token e url de envio do número
      try {
        if (ticket.whatsappId) {
          ...
          extraWhatsappData = {
            ...
            whatsappSendToken: whatsapp.send_token || null,
            whatsappApiSendUrl: `${process.env.BACKEND_URL}/api/messages/send`
          };
        }
      } catch (err) {
        console.error("Erro ao buscar dados do Whatsapp para webhook N8N:", err);
      }
      ...
      const messageData = {
        ...
        ticketData: {
          ...
          typebotSessionTime: ticket.typebotSessionTime,
      ...
      console.error("Erro geral no processamento do webhook N8N:", error);
      throw new Error(error);
    }
    // --- FIM LÓGICA WEBHOOKS_N8N ---
